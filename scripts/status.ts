 ...
 *
 * Usage:
 *   bun run skill-dev/skill-v2-lab/scripts/status.ts '<source_id>' '<JSON event data>'
 *
 ...
 *
 * Examples:
 *   bun run skill-dev/skill-v2-lab/scripts/status.ts 'abc123' '{ "event_type": "status", "data": { "message": "Extracting transcript..." } }'
 *   bun run skill-dev/skill-v2-lab/scripts/status.ts 'abc123' '{ "event_type": "thesis_found", "data": { "message": "Google zero profit in 2027", "progress": 1, "total": 5 } }'
 *   bun run skill-dev/skill-v2-lab/scripts/status.ts 'abc123' '{ "event_type": "complete", "data": { "message": "All trades posted", "source_theses": [...] } }'
 */
...
if (!sourceId || !eventPayload) {
  console.error("Usage: bun run skill-dev/skill-v2-lab/scripts/status.ts '<source_id>' '<JSON event>'");
  process.exit(1);
}
