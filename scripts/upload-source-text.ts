 ...
 *
 * Usage:
 *   bun run skill-dev/skill-v2-lab/scripts/upload-source-text.ts <source_id> --file <path> [--provider transcript]
 *   bun run skill-dev/skill-v2-lab/scripts/upload-source-text.ts <source_id> '{"raw_text":"...","provider":"transcript"}'
 */
...
if (!sourceId) {
  console.error("Usage: bun run skill-dev/skill-v2-lab/scripts/upload-source-text.ts <source_id> (--file <path> | '<JSON payload>')");
  process.exit(1);
}
