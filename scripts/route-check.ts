#!/usr/bin/env bun
/**
 * Backend-only route-check adapter.
 *
...
const PASTE_TRADE_KEY = process.env.PASTE_TRADE_KEY || process.env.PASTE_TRADE_API_KEY || "";
...
function parseArgs(argv: string[]) {
  ...
  if (filteredArgs.length < 2) {
    console.error("Usage: bun run skill-dev/skill-v2-lab/scripts/route-check.ts [--run-id <runId>] <TICKER[,TICKER]> <long|short> [options]");
    ...
    console.error("  --raw                      Print backend assess payload without route-check summary shaping");
    process.exit(1);
  }
  ...
  return { tickers, direction, sourceDate, capital, horizon, subjectKind, runId, outputMode };
}

function assertBackendResponse(payload: unknown): asserts payload is BackendAssessResponse {
  if (!payload || typeof payload !== "object") {
    throw new Error("Malformed backend assess response: expected JSON object.");
  }
  const record = payload as Record<string, unknown>;
  if (!Array.isArray(record.results)) {
    throw new Error("Malformed backend assess response: missing results array.");
  }
  if (typeof record.contract_version !== "string") {
    throw new Error("Malformed backend assess response: missing contract_version.");
  }
}

async function callBackendAssess(
  tickers: string[],
  direction: Direction,
  capital: number,
  ...
  if (!PASTE_TRADE_KEY) {
    throw new Error("PASTE_TRADE_KEY (or PASTE_TRADE_API_KEY) is required for route-check adapter.");
  }

  const response = await fetch(`${PASTE_TRADE_API}/api/skill/assess`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    let message = `Backend route-check failed (${response.status})`;
    try {
...
function toCandidateRoutes(perps: PerpInstrument | null): RouteCheckSummary["candidate_routes"] {
...
function buildSummary(item: TickerAssessment): RouteCheckSummary {
  ...
  let selected: RouteCheckSummary["selected_expression"] = {
    platform: null,
    instrument: null,
    routed_ticker: null,
    entry_price: null,
  ...
  if (perpsAvailable) {
    selected = {
      ...
      entry_price: toFiniteNumber(perps?.entry_price) ?? toFiniteNumber(item.current_price),
    };
    ...
    selected = {
      ...
      entry_price: toFiniteNumber(shares?.entry_price) ?? toFiniteNumber(item.current_price),
    };
  }
...
export async function runRouteCheckCli(argv = process.argv): Promise<void> {
  const { tickers, direction, sourceDate, capital, horizon, subjectKind, runId, outputMode } = parseArgs(argv);

  console.error(
    `\nRoute-check ${tickers.join(", ")} ${direction} | $${capital.toLocaleString()} capital`
  ...
  const backend = await callBackendAssess(
    tickers,
    direction,
    capital,
    sourceDate,
